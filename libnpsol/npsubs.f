...
*     NPFILE   NPGETR   NPIQP    NPKEY    NPLOC    NPMRT    NPOPTN
*     NPPRT    NPRSET   NPSAVR   NPSETX   NPSRCH   NPUPDT   NPSOL
*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
*     This version of NPCORE dated 26-Nov-1989.
************************************************************************
...
*        ===============================================================
  100    IF (MOD(MAJITS,KSAVE) .EQ. 0 .AND. MAJITS .NE. MAJIT0) THEN
            CALL NPSAVR( UNITQ, N, NCLIN, NCNLN, LDR, LDZY,
     $                   NFREE, NSAVE, MAJITS, ISTATE, KX,
     $                   W(LHFRWD), W(LHCTRL),
     $                   W(LCMUL), R, W(LRHO), X, W(LZY) )
         END IF
...
*                 ------------------------------------------------------
     ...
     $               WRITE (NOUT, 9000) RFROBN, DRMAX, DRMIN,COND,RCNDBD
                  ...
                  LSUMRY(5:5) = 'Refactorize Hessian'
...
*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      SUBROUTINE NPGETR( INFORM, UNITQ, N, NCLIN, NCNLN, LDR, LDZY,
     ...
     $                   CMUL, R, RHO, X, ZY )
      ...
      DOUBLE PRECISION   R(LDR,*), X(N), ZY(LDZY,*)
      DOUBLE PRECISION   HFORWD(*), HCNTRL(*)
...
*        K1  through  K2.
     ...
     $                   N, LDZY, LDAQP, LDR, LDT,
     $                   ISTATE, KACTIV, KX, CONDMX,
     $                   AQP, R, W(LT), W(LRPQ), W(LGQ), W(LZY),
     $                   W(LWRK1), DX, W(LRLAM) )
      END IF
...
*     ==================================================================
     ...
     $             N, NLNX, NCTOTL, LDZY, LDAQP, LDR, LDT,
     $             ISTATE, KACTIV, KX,
     $             DXNORM, GDX,
...
*     objective function and  R(pq)  from the transformed residual.
      ...
      QPCURV = TWO*SSQ
      IF ( QPCURV .LE. EPSPT9 ) QPCURV = EPSPT9
...
*              Recompute RPQ, HPQ, GDX and QPCURV.
     ...
     $                        DX(NLNX+1), 1, ONE, W(LRPQ), 1 )
               ...
               CALL CMQMUL( 3, N, NZ, NFREE, LDZY, UNITQ,
     $                      KX, DX, W(LZY), W(LWRK1) )
...
*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     $                   GQ, R, ZY, WORK, QRWORK )
      ...
      DOUBLE PRECISION   GQ(N), R(LDR,*), ZY(LDZY,*)
...
*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      SUBROUTINE NPSAVR( UNITQ, N, NCLIN, NCNLN, LDR, LDZY,
...
*     than that at the base point. If  INFORM = 4, 5, 6 or 7,  ALFA  is
*     zero and  OBJALF will be the merit value at the base point.
*
*     Original version written  27-May-1985.
*     Level 2 BLAS added 12-June-1986.
*     This version of NPSRCH dated  10-May-1990. 
************************************************************************
      DOUBLE PRECISION   WMACH
      COMMON    /SOLMCH/ WMACH(15)
      SAVE      /SOLMCH/
...
*     ------------------------------------------------------------------
      ...
      TOLABS = ALFMAX
      IF (TOLRX*XNORM + TOLAX .LT. DXNORM*ALFMAX)
...
*     ==================================================================
      IF (NLOAD .GT. 0) THEN
         CALL NPGETR( NERROR, UNITQ, N, NCLIN, NCNLN, LDR, LDZY,
