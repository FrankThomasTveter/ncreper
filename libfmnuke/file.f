#__file: 'addgroup.F' 0100664    **DO NOT DELETE**
...
C     
      RETURN
      END
#__file: 'addgrp.F' 0100664    **DO NOT DELETE**
...
C
      ...
      RETURN
      END
#__file: 'chop.F' 0100664    **DO NOT DELETE**
...
C
      ...
      DO WHILE(II.LE.LENS)
C     REPLACE [] WITH ()
         IF (STRING(II:II).EQ.'['.and.
     &        string(max(1,ii-1):max(1,ii-1)).ne.'\\') THEN
            BUFF='('
            CALL SUBSTR(STRING,N,BUFF,1,II,DELL)
         ELSEIF (STRING(II:II).EQ.']'.and.
     &        string(max(1,ii-1):max(1,ii-1)).ne.'\\') THEN
            BUFF=')'
            CALL SUBSTR(STRING,N,BUFF,1,II,DELL)
         ENDIF
C     REMOVE 'PROTECTIVE BRACKETS' {}, OR GOTO NEXT CHARACTER
         IF (STRING(II:II).EQ.'{'.OR.STRING(II:II).EQ.'}'.and.
     &        string(max(1,ii-1):max(1,ii-1)).ne.'\\') THEN
...
C     --------------------------------------------------------------------- *
C     | F. TVETER    | 18/07/95 | NEW                                     | *
C     | S.EASTWOOD   | 10/12/07 | CHANGED NINT(RRR) TO NINT(RRR(1))       | *
C     |              |          |                                         | *
C     --------------------------------------------------------------------- *
C     ***********************************************************************
C
...
C
      ...
      REAL PI,E,XMU,RE,RAD,DEG,RRR(2),RR,SEC,TIME
      LOGICAL FIRST
      DATA FIRST /.TRUE./
C
      CHARACTER*8 MYNAME
...
C
      ...
      RETURN
      END
#__file: 'findpos.F' 0100664    **DO NOT DELETE**
...
C
     ...
     &        string(max(1,ii-1):max(1,ii-1)).ne.'\\') THEN
            ...
            IF (LEVEL.GT.MAXLEV) THEN
               ...
               MFOUND(2)=.FALSE.
            ENDIF
         ELSEIF(STRING(II:II).EQ.DEL(2).and.
     &        string(max(1,ii-1):max(1,ii-1)).ne.'\\') THEN
...
C
      RETURN
      END
#__file: 'nuked.F' 0100640    **DO NOT DELETE**
      SUBROUTINE NUKED(KODE,UNITI,FILENM,HDR250,INTOUT,DAT250,
     &     ENDOFF,IRC)
C     ***********************************************************************
C     +                                                                     *
...
C     KODE=-1                                                               *
C     CALL NUKED(KODE,UNITI,HDR250,INTOUT,DAT250,ENDOFF,IRC)                *
C     KODE=0                                                                *
C     CALL NUKED(KODE,UNITI,HDR250,INTOUT,DAT250,ENDOFF,IRC)                *
C     KODE=+1                                                               *
C     CALL NUKED(KODE,UNITI,HDR250,INTOUT,DAT250,ENDOFF,IRC)                *
...
C
               IF (STRING(JJ:JJ).EQ.'('.and.string(max(1,jj-1)).ne.'\\') 
     &              LEVEL=LEVEL+1
               IF (STRING(JJ:JJ).EQ.')'.and.string(max(1,jj-1)).ne.'\\') 
     &              LEVEL=LEVEL-1
               IF (STRING(JJ:JJ).EQ.'['.and.string(max(1,jj-1)).ne.'\\') 
...
C
      RETURN
      END
#__file: 'ord1bid.F' 0100664    **DO NOT DELETE**
...
#__file: 'qcom.F' 0100664    **DO NOT DELETE**
     ...
     &     BUFF1000,LENB,PROCESS,BDONE,ENDOFF,LDUMP,IRC)
C
C     ***********************************************************************
C     WRITTEN/MODIFIED BY:                                                  *
C     --------------------------------------------------------------------- *
...
C     
     ...
     &     FOUND, COMP, ACTIVE
      ...
      EXTERNAL NUKEHEAD, LENGTH
      CHARACTER*1 DEL(2),C1
      INTEGER POS(2),II,JJ,KK,LEV,IFLVL,CLVL
      CHARACTER*8 MYNAME
      DATA MYNAME /'QCOM'/
      INTEGER FTUNIT
      CHARACTER*250 DUMP250
C
C     EQUAL SIGN IDENTIFIERS
C
      INTEGER MAXEQN
...
C
     ...
     &              buff1000(ii-1:ii).eq.'\\)') then
            ...
            call system(BUFF1000(2:LENB),STATUS=irc)
            IF (IRC.NE.0) THEN
...
C     
     ...
     &           '----------------------------------------'
         ELSEIF (COMMAND.EQ.13 .AND. PROCESS) THEN ! help
            ...
            write(*,*)myname,'Processing ',command,process
            PROCESS=.FALSE.
C     LIST ALL HEADERS
            WRITE(*,*) MYNAME,
     &           '--------LIST OF POSSIBLE COMMANDS-------'
     ...
     &              WRITE(*,*) HDR250(II)(1:LENS)
            ...
            WRITE(*,*) MYNAME,
     &           '----------------------------------------'
         ELSEIF (COMMAND.EQ.14 .AND. PROCESS) THEN ! dump
            PROCESS=.FALSE.
            DEL(1)='('
            DEL(2)=')'
            LEV=0
            ...
            IF (IRC.NE.0.OR.LEV.EQ.-1) THEN  ! syntax error
               ERROR=.TRUE.
               BUFF1000='missing () in dump statement.'
            ELSE
               PATHF=BUFF1000(POS(1)+1:POS(2)-1)
               CALL CHOP(PATHF,LENB)
               LENF=LENGTH(PATHF,250,10)
               ...
               END IF
C     
               OPEN(UNIT=LDUMP,FILE=PATHF(1:LENF),
     &              ACCESS='SEQUENTIAL',FORM='FORMATTED',
     &              STATUS='UNKNOWN',IOSTAT=IRC)
               IF (IRC.NE.0) THEN
                  WRITE(*,*) myname,'Unable to open:'//PATHF(1:LENF),
     7                 LDUMP
                  RETURN
               END IF
            ENDIF
...
C
      IRC=0
      CALL CHOP(STRING1,250)
      CALL CHOP(STRING2,250)
...
C
      RETURN
      END
#__file: 'trunc0.F' 0100664    **DO NOT DELETE**
...
C
      RETURN
      END
#__file: 'trunc.F' 0100664    **DO NOT DELETE**
...
C
     ...
     &           .OR.STRING(POS(1)-1:POS(1)-1).EQ.'[') THEN
               POS(1)=POS(1)-1
               IF (POS(1).GT.1) THEN
                  IF(STRING(POS(1)-1:POS(1)-1).EQ.'-'
     &                 .OR.STRING(POS(1)-1:POS(1)-1).EQ.'+') THEN
     ...
     &              .OR.STRING(POS(1)-1:POS(1)-1).EQ.'+').AND..NOT.
     &              (STRING(POS(1):POS(1)).EQ.'-'
     &              .OR.STRING(POS(1):POS(1)).EQ.'+')) THEN
               POS(1)=POS(1)-1
            ENDIF
         ENDIF
         POS(2)=START
     ...
     &           .OR.STRING(POS(2)+1:POS(2)+1).EQ.']') POS(2)=POS(2)+1
         ENDIF
      ENDIF
C
      CODE=ICODE
